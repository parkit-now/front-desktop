...
if (!target || !['mac', 'win', 'linux'].includes(target)) {
  console.error(`Error: target desconocido "${arg ?? process.platform}". Uso: node scripts/check-build-env.mjs [mac|win|linux]`);
  process.exit(1);
}
...
const defaultPyBin = process.env.PYTHON_BIN || (target === 'win' ? 'python' : 'python3.12');
...
if (!pyBin) {
  const alt = [defaultPyBin, 'python3.12', 'python3', 'python'].find((c) => is312(run(c, ['--version'])));
  if (alt) {
    pyBin = alt;
    warn(`"${defaultPyBin}" no está en el PATH — usá:  make dist-${target} PYTHON_BIN=${alt}`);
  }
}
...
if (!pyVer) {
  ...
  else warn(`${pyVer} (${pyBin}) — el proyecto fija Python 3.12; otra minor puede fallar al resolver wheels (onnxruntime, opencv)`);
  ...
  else err(`${pyBin} no tiene venv/ensurepip — en Debian/Ubuntu: sudo apt install python3.12-venv`);
  ...
  if (target === 'win' && /WindowsApps/i.test(pyExe)) {
    err('`python` apunta al stub de Microsoft Store — instalá Python 3.12 desde python.org y desactivá el alias en "Configuración → Alias de ejecución de aplicaciones"');
  }
}
...
else err('bash no encontrado — los Makefiles de services/* lo necesitan (en Windows: Git Bash o WSL)');

// ── GNU make ─────────────────────────────────────────────────────────────────
const mk = run('make', ['--version']);
if (mk && /GNU Make/.test(mk)) ok(mk.split('\n')[0]);
else if (mk) warn('make encontrado pero no parece GNU Make — algunos targets usan sintaxis GNU');
else warn('no pude verificar make (raro si estás leyendo esto vía `make doctor`)');

// ── upx (opcional) ───────────────────────────────────────────────────────────
const upx = run('upx', ['--version']);
if (upx) ok(`upx ${upx.split('\n')[0].replace(/^upx\s+/i, '')}`);
else warn('upx no encontrado — los build.spec tienen upx=True; PyInstaller lo saltea (binarios más pesados, no es fatal)');
...
else warn('.env ausente — corré `make env-use-prod` (o `env-use-local`) antes de empaquetar');
...
if (target === 'win') {
  ...
  if (existsSync(vswhere)) {
    ...
    else warn('VS Build Tools sin workload "Desktop development with C++" — solo hace falta si @electron/rebuild compila un módulo nativo');
  } else {
    warn('no detecté VS Build Tools — solo hace falta si @electron/rebuild compila un módulo nativo');
  }
}
